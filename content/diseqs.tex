...
The block disequalities are stored as a map $B[\Psi]$ which maps representative terms to a set of representative terms that are not in the same block.
It is sufficient to only store the disequalities between representatives, i.e.,
the normalized disequalities $norm_{P[\Psi]}(p)$.
The block disequalities between other terms can be derived from the congruence closure of $\Psi$ and the map $B[\Psi]$.
...
The set of disequalities following from rule~\ref{item:eq-neq} and rule~\ref{item:block-neq} can be infinite, but we don't need to explicitly store them, as they implicitly derive from the relations $\equivp$ and $bl(\cdot)\nequivp bl(\cdot)$, which are stored in the partition $P[\Psi]$ and the map $B[\Psi]$.
...
Then
it holds that $\Psi$ implies $p$ iff $t_1 \nequivp z + t_2$  and iff one of the following holds:
