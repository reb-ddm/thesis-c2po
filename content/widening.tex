...
\begin{example}
  ...
  Then we have for each $n \geq 0$, $\Psi_n \Longrightarrow \Psi_{n+1}$ and in particular the product automaton between $M_P$\todo{what is P} and $M_P[\Psi_{n+1}]$ is $M_P[\Psi_{n+1}]$.
...
\begin{example}\label{e:narrow1}\todo{rephrase, this is copied from the paper}
...
\todo[inline]{find smallest and biggest offset in the disequalities and only consider offsets in this range}
...
We also filter out the disequalities that contain offsets that are not in the range of offsets of the first input conjunction.\todo{explain more in detail}
