...
\begin{example}\label{ex:min-repr}
    Let $\Psi \equiv B = -1 + \&x \land *B = 2 + A \land *A = 3 + x$.\todo{don't use B}
    The automaton $M$\todo{what is P} has can be visualized with the following graph:
    \begin{center}
...
\begin{example}
    ...
    The normal form of the kernel $k$ representing $\Psi$ is\todo{please no B}
    \[
