...
\begin{example}
    ...
    We want to compute $\restr{M_P}{\Set}$.
    It holds that $\T \cap \Set = \{\&y, \&z\}$.
    The set $\T \cap \Set$ does
    not contain any terms from $\L_{M_P}(s_2) = \{x,z\}$.
    However, $\L_{M_P}(s_2)$ contains $z$, which is in $\Set$, and is therefore allowed to appear in $\restr{M_P}{\Set}$.
...
The restriction $\restr{M_P}{\Set} = (S', \otau', \eta', \delta')$ is computed using a breadth-first search (BFS) on the automaton $M_P$ to determine which paths remain reachable when keeping only the terms in $\Set$.
