...
\begin{example}\label{example:infinite-join}\todo{nooo please no B}
...
\begin{example}\label{join-example}
    ...
    First, we add the terms of the set $\T = \{A, B, *A,*B,*(1+A)$, $*(1 + B)\}$ to the automaton $M_1$, resulting in the automata $M_1'$.
    Let $M_3$ be the product automata of $M_1'$ and $M_2$.
    The automata $M_1'$ and $M_3$ can be visualized as follows:
        ...
        \begin{minipage}{0.45\textwidth}
            ...
            \begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto, state/.style={rectangle, rounded corners, draw, inner sep=3pt, align=center}]
                ...
                \node[state] (Q1) {$\begin{array}{c}
                            A, B
                        \end{array}$};
                \node[state, below=1.3cm of Q1] (Q2) {$\begin{array}{c}
                            *(1+A), *(1+B)
                        \end{array}$};
                \node[state, below left=0.5cm and 1.4cm of Q1] (Q3) {$\begin{array}{c}
                            *A
                        \end{array}$};
                \node[state, below right=0.5cm and 1.4cm of Q1] (Q4) {$\begin{array}{c}
                            *B
                        \end{array}$};
                \path[->]
                (Q1) edge node {$1,0$} (Q2)
                ...
                \node [below left=-0.2cm and 1.2cm of Q1] {$M_1'$:};
            \end{tikzpicture}
        \end{minipage}
        \begin{minipage}{0.45\textwidth}
            \centering
            \begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto, state/.style={rectangle, rounded corners, draw, inner sep=5pt, align=center}]
                ...
                \node[state, right=1.4cm of Q1] (Q2) {$\begin{array}{c}
                            B
                        \end{array}$};

                \node[state, below right=1.8cm and 0.9cm of Q1] (Q3) {$\begin{array}{c}
                            *(1+A), *(1 + B)
                        \end{array}$};
                \node[state, below left=01.1cm and 1.8cm of Q1] (Q4) {$\begin{array}{c}
                            *A
                        \end{array}$};
                \node[state, below right=1.1cm and 1.8cm of Q2] (Q5) {$\begin{array}{c}
                            *B
                        \end{array}$};
                \path[->]
                (Q1) edge[] node[left] {$1,0$} (Q3)
                (Q2) edge[] node[right] {$1,0$} (Q3)
                (Q1) edge[] node[above left=-0.1cm and -0.1cm] {$1,0$} (Q4)
                (Q2) edge[] node[above] {$1,0$} (Q5);

                \node [below left=-0.2cm and 1cm of Q1] {$M_3$:};

            \end{tikzpicture}
        \end{minipage}
    \end{center}
    From $M_3$ we can infer the equality $*(1 + A) = *(1 + B)$, while the equalities $*(1 + *^n A) = *(1 + *^n B)$ for $n \geq 1$ are implied by $M_1'$ and $M_2$, but not by $M_3$.
\end{example}
