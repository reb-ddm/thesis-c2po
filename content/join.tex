...
\begin{example}\label{example:infinite-join}
    Adapting the original example by \textcite{join} to \cpo, consider the conjunctions $\Psi_1 \equiv (A = V)$ and $\Psi_2 \equiv (*A = A)\land (*V=V) \land (*(1 + A) = *(1 + V))$, represented by the automata $M_1$ and $M_2$ shown below:
    \begin{center}
