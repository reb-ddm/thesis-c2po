  ...
  \item Case 2: $\tau\,t_1 \neq \tau\,t_2$. We call $\union{t_1}{t_2}{z}$.
        ...
        \begin{itemize}
          \item Let $\otau\,s_1 = \tau\,t_1$ and $\otau\,s_2 = \tau\,t_2$.
                After the merge, the state $s_2$ will not be in $S'$, as the equivalence classes of $s_1$ and $s_2$ are merged.
                Therefore, we need to move all outgoing transitions of $s_2$ to be transitions of $s_2$.
                For each transition $\delta(z_2, s_2) = (z_3, s_3)$, we set $\delta'(z_1, s_1) = (z_3, s_3)$, where $z_1 = z_2 - \omega\,t_1 + \omega\,t_2 - z$.
...
\begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto, state/.style={rectangle, rounded corners, draw, inner sep=3pt, align=center}]
  ...
  \node[state, right of=B] (Bs) {$*B$};

  \path[->]
  (A) edge[] node {$0,0$} (As)
  (B) edge[] node {$0,0$} (Bs);

  \node[above left=1cm and 0.3cm of A] {$s_0$};
  \node[above left=1cm and 0.5cm of B] {$s_1$};
  \node[above left=1cm and 0.7cm of As] {$s_2$};
  \node[above left=1cm and 0.7cm of Bs] {$s_3$};

\end{tikzpicture}

\begin{example}
  A QFA can be visualized by labeling the states with the terms of $T$ that are in the corresponding equivalence class.
  For example, consider the set of terms $\T = \{A, B, *A, *B\}$.
  The initial automaton would be the one in figure \ref{fig:initial-example}.
  If the operation $\closure{A}{1}{B}{P}$ is called, the automaton would be updated as in figure \ref{fig:closure-example}.
