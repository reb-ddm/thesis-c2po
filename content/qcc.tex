...
The QFA is constructed from the partition  $(\T, \tau, \omega)$ by defining $M_P = (S, \eta, \delta)$ where $S$ contains a state $s_i$ for each equivalence class of $P$,
...
\begin{theorem}
  ...
  The corresponding quantitative partition $(\T, \tau,\omega)$ and a corresponding QFA $M_P = (S, \eta, \delta)$ are constructed by applying the closure operation to all equalities in $\Psi$.
