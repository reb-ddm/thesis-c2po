...
\begin{example}
  ...
  \begin{figure}
    ...
    \end{tikzpicture}
    \caption{Union-find tree corresponding to the conjunction  $(V = 1 + A) \land (D = 2 + \&x) \land (A = 3 + \&x)$}\label{fig:uf-tree}
  \end{figure}
...
\begin{centering}
  \begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, auto, state/.style={rectangle, rounded corners, draw, inner sep=5pt, align=center}]
    ...
    \node[state, below=1cm of A] (V) {$V$};

    \node[state, right of=A] (As) {$*A$};

    \node[state, right of=V] (Vs) {$*(1 + V)$};
    ...
    \node[above left=0.5cm and 0.3cm of V] {$s_1$};
    \node[above left=0.5cm and 0.3cm of As] {$s_2$};
    \node[above left=0.5cm and 0.3cm of Vs] {$s_3$};
...
\end{centering}
  ...
  \caption{The same QFA after the operation $\closure{A}{1}{V}{P}$.}\label{fig:closure-example}
\end{subfigure}

\caption{An example of the $\closure{A}{1}{V}{P}$ operation on a QFA over the set of terms
$\T = \{A, V, *A, *(1 + V)\}$.
...
\begin{example}
  ...
  For example, consider the set of terms $\T = \{A, V, *A, *(1 + V)\}$.
  ...
  If the operation $\closure{A}{1}{V}{P}$ is called,
  first the $\union{A}{1}{V}$ is performed and the new representative of $V$ becomes $A$, with offset $1$.
  It would be equivalent to choose $V$ as the new representative, but we choose $A$ here as an example.
  ...
  Therefore, we add the equality $*(1 + V) = *A$ to a queue of pending equalities.
  Then we recursively call $\closure{*(1 + V)}{0}{*A}{P'}$ and thus perform the union of $*(1 + V)$ and $*A$.
  As before, we choose $*A$ as the new representative, but $*(1+V)$ would also be a possible choice.
