...
In the following, we will describe the abstract semantics $\sem{s_1\,{:=}\,s_2}^\sharp\,k$ of the assignment $s_1\,{:=}\,s_2$ over the kernel $k$, for the three different types of assignments.
...
\ignore{$P = (\T, \tau, \omega)$, a QFA $M_P = (S, \otau, \eta, \delta)$, }
...
\todo[inline]{ As simplification we call the restricted kernel $\restr{k}{\neg t}$. It represents all equalities that follow from $M$ and that are still valid after overwriting the value of $t$.}
