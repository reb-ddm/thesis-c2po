...
Before diving into the abstract effects of a function call, we discuss a concrete semantic for function calls.
...
\begin{figure}
  ...
  \begin{tikzpicture}[
    ...
    \node[block, below right=0.5cm and 4cm of usw1, initial right, initial text=f()] (fstart) {$p_4$};
    ...
    \draw[call, bend left=30] (beforecall.east) to node[above] {$\enter(C[p_1])$} (fstart.west);
    \draw[combine] (beforecall) ++(0,-1.1cm) .. controls +(right:1cm) and +(down:0.5cm) .. (aftercall.east);
    \draw[->, combine, bend left=30] (fend.west) to node[below] {$\combine(C[p_1],C[p_5])$} (aftercall.east);
