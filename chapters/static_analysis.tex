...
The state of node $p_3$ is equal to $s_{p_3} = \sem{x = x+1}\,s_{p_2} = \angl{x \rightarrow 2}$.\todo{$s_p$ is ugly. Find something else, like $X[p]$ for the abstract semantics}
...
We discuss a concrete semantic for functions before diving into the abstract effects of a function call.
The concrete semantics of a function depend not only on the function definition but also on the values of the parameters, global variables, and the state of the memory at the time when the function is called.
